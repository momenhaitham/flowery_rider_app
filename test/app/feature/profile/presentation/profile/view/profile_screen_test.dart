...
void main() {
  ...
  testWidgets('test profile init state with loading', (widgetTester) async {
    when(
      viewModel.state,
    ).thenReturn(ProfileState(profileState: BaseState(isLoading: true)));
    ...
    when(viewModel.cubitStream).thenAnswer(
      (_) => Stream<ProfileEvent>.value(
        NavigateToEditProfileOrVehicleScreen(driverEntity),
      ),
    );
  ...
  testWidgets('test profile with error in getting profile data', (
    widgetTester,
  ) async {
    when(viewModel.state).thenReturn(
      ProfileState(
        profileState: BaseState(isLoading: false, error: ConnectionError()),
      ),
    );
    when(viewModel.stream).thenAnswer(
      (_) => Stream<ProfileState>.value(
  ...
  testWidgets('test profile with success in getting profile data', (
    widgetTester,
  ) async {
    when(viewModel.state).thenReturn(
      ProfileState(
    ...
    expect(
      find.text('${driverEntity.firstName} ${driverEntity.lastName}'),
      findsOneWidget,
    );
