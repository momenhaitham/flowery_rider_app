...
void main() {
  ...
  testWidgets('test change password  init state ',(widgetTester)async {
    when(viewModel.state).thenReturn(ChangePasswordState(changePasswordState: BaseState()));
    when(viewModel.stream).thenAnswer((_)=>Stream<ChangePasswordState>.value(ChangePasswordState(changePasswordState: BaseState())));
    when(viewModel.cubitStream).thenAnswer((_)=>Stream<ChangePasswordEvent>.value(BackToEditProfileNavigationEvent()));
  ...
  testWidgets('test change password  when tapping with empty fields ',(widgetTester)async {
    when(viewModel.state).thenReturn(ChangePasswordState(changePasswordState: BaseState()));
    when(viewModel.stream).thenAnswer((_)=>Stream<ChangePasswordState>.value(ChangePasswordState(changePasswordState: BaseState())));
    when(viewModel.cubitStream).thenAnswer((_)=>Stream<ChangePasswordEvent>.value(BackToEditProfileNavigationEvent()));
    await widgetTester.pumpWidget(buildWidget());
  ...
  testWidgets('test change password  when tapping with wrong password formate ',(widgetTester)async {
    when(viewModel.state).thenReturn(ChangePasswordState(changePasswordState: BaseState()));
    when(viewModel.stream).thenAnswer((_)=>Stream<ChangePasswordState>.value(ChangePasswordState(changePasswordState: BaseState())));
    when(viewModel.cubitStream).thenAnswer((_)=>Stream<ChangePasswordEvent>.value(BackToEditProfileNavigationEvent()));
    await widgetTester.pumpWidget(buildWidget());
    await widgetTester.enterText(find.byKey(Key('current_password_text_field')), '123');
    await widgetTester.tap(find.byType(ElevatedButton));
    await widgetTester.pump();
    expect(find.byType(TextFormField),findsNWidgets(3));
    expect(find.byKey(Key('change_password_padding')),findsOneWidget);
  ...
  testWidgets('test change password  when tapping with not found password',(widgetTester)async {
    when(viewModel.state).thenReturn(ChangePasswordState(changePasswordState: BaseState(
      error:ServerError(message: 'not found'),isLoading: true
    )));
    when(viewModel.stream).thenAnswer((_)=>Stream<ChangePasswordState>.value(ChangePasswordState(changePasswordState: BaseState(
      error:ServerError(message: 'not found'),isLoading: true
    ))));
    when(viewModel.cubitStream).thenAnswer((_)=>Stream<ChangePasswordEvent>.value(BackToEditProfileNavigationEvent()));
    await widgetTester.pumpWidget(buildWidget());
    await widgetTester.enterText(find.byKey(Key('current_password_text_field')), 'Sayed@1234');
    await widgetTester.enterText(find.byKey(Key('new_password_text_field')), 'Sayed@123');
    await widgetTester.enterText(find.byKey(Key('confirm_password_text_field')), 'Sayed@123');
    await widgetTester.tap(find.byType(ElevatedButton));
    await widgetTester.pump();
    expect(find.text('not found'),findsOneWidget);
    expect(find.byType(CircularProgressIndicator),findsOneWidget);
    },);
  testWidgets('test change password  when tapping with succes',(widgetTester)async {
    print(find.byKey(Key('current_password_text_field')).evaluate().length);
    when(viewModel.state).thenReturn(ChangePasswordState(changePasswordState: BaseState(
        data: 'success',isLoading: true
    )));
    when(viewModel.stream).thenAnswer((_)=>Stream<ChangePasswordState>.value(ChangePasswordState(changePasswordState: BaseState(
        data: 'success',isLoading: true
    ))));
    ...
    expect(find.text(AppLocale.passwordChanged),findsOneWidget);
    expect(find.byType(CircularProgressIndicator),findsOneWidget);
