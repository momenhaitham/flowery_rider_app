...
void main() {
  ...
  testWidgets('test update vehicle init state with loading', (
    ...
    when(viewModel.cubitStream).thenAnswer(
      (_) => Stream<UpdateProfileEvent>.value(NavigateToProfileEvent()),
    );
  ...
  testWidgets('test update vehicle success state ', (widgetTester) async {
    when(viewModel.state).thenReturn(
      UpdateProfileState(
        profileState: BaseState(data: 'success'),
        profilePhotoState: BaseState(isLoading: true),
        vehiclesState: BaseState(isLoading: true),
      ),
    );
  ...
  testWidgets('test update vehicle error state ', (widgetTester) async {
    when(viewModel.state).thenReturn(
      UpdateProfileState(
        profileState: BaseState(error: UnexpectedError()),
        profilePhotoState: BaseState(isLoading: true),
        vehiclesState: BaseState(isLoading: true),
      ),
    );
    ...
    expect(find.byType(Text), findsNWidgets(8));
    expect(
      find.text('${AppLocale.update_profile_error} some thing went error'),
      findsOneWidget,
    );
    expect(find.byType(SnackBar), findsOneWidget);
  });
